  ...
  apiPack.setAPIFunctionExecute(function(serverPack, apiResponse, completion) {
    ...
    serverPack.alchemy_language.sentiment(serverPack.alchemy_params, function (err, response) {
      ...
      if (err) {
        ...
        response.docSentiment.scoreRange = "returns values between 0 and 1";
        apiResponse.result.score = normalizeSentimentScore((response.docSentiment.score * 100) * multiplier);
  ...
  apiPack.setAPIFunctionExecute(function(serverPack, apiResponse, completion) {
    ...
    doc.detectSentiment(config, function (err, sentiment) {
        ...
        if (err) {
          ...
          sentiment.scoreRange = "returns values between 0 and 100, modified by polarity.";
          apiResponse.result.score = normalizeSentimentScore(sentiment.magnitude * multiplier);
